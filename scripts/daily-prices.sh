...
# --no-proxy disable proxy (proxy has issues with this job)
# This updates prices for existing cards (doesn't mess with names)
echo "💰 Discovering and updating market prices..."
timeout 3600 node dist/index.js discover-by-price --headless --min-price 9 --max-pages 100 --no-proxy 2>&1 || {
    EXIT_CODE=$?
